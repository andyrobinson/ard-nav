...
void SelfTest::test() {
  ...
  rudder->set_position(45);
  timer->wait(SERVO_WAIT);
  logger->msg("Tst: Rudder complete");
  ...
  allok = (allok && ok);
  sprintf(logmsg, "Tst: Wind %s", ok?pass:fail);
  logger->msg(logmsg);
  ...
  allok = (allok && ok);
  sprintf(logmsg, "Tst: Compass %s", ok?pass:fail);
  logger->msg(logmsg);
  ...
  allok = (allok && ok);
  sprintf(logmsg, "Tst: Gps %s", ok?pass:fail);
  logger->msg(logmsg);

  next();
  logger->msg("Tst: Complete");
