...
TEST_F(SailTest, Stub_servo_should_record_last_position) {
  MServo servo;
  servo.write(3,88);
  EXPECT_EQ(servo.angle_last_called, 88);
  EXPECT_EQ(servo.channel_last_called, 3);
}

TEST_F(SailTest, Should_initialise_speeed_and_accel) {
  sail.begin();
  EXPECT_EQ(stub_servo.channel_last_called, 1);
  EXPECT_EQ(stub_servo.speed_last_called, SAIL_SPEED);
  EXPECT_EQ(stub_servo.accel_last_called, SAIL_ACCEL);
}

TEST_F(SailTest, Should_Put_Sail_Straight_When_In_Irons) {
  sail.set_position(0);
  EXPECT_EQ(stub_servo.angle_last_called, servo_position(0));
}
...
TEST_F(SailTest, Should_maintain_angle_of_attack_when_close_hauled) {
  ...
  for (short i = NO_GO_LIMIT; i <= PURE_LIFT_LIMIT; i++) {
    sail.set_position(i);
    EXPECT_EQ(stub_servo.angle_last_called,servo_position(i - ANGLE_OF_ATTACK));
...
TEST_F(SailTest, Should_fix_sail_allowing_Wind_to_increase_angle_of_attack_moving_from_lift_to_drag) {
  ...
  for (short i = PURE_LIFT_LIMIT; i <= LIFT_TO_DRAG_LIMIT; i++) {
    sail.set_position(i);
    EXPECT_EQ(stub_servo.angle_last_called,servo_position(fixed_sail));
...
TEST_F(SailTest, Should_increase_drag_after_lift_to_drag_limit) {
  ...
  for (short i = LIFT_TO_DRAG_LIMIT; i <= 180 - GYBE_CHECK_MAX_DIFF; i++) {
    ...
    sail.set_position(i);
    EXPECT_EQ(stub_servo.angle_last_called,servo_position(expected_position));
...
TEST_F(SailTest, Should_not_increase_sail_angle_beyond_90) {
  ...
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(-90));
...
TEST_F(SailTest, Should_not_gybe_but_set_to_max_for_10_degrees_when_running) {
  ...
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(82));
...
TEST_F(SailTest, Should_not_gybe_if_still_on_same_side) {
  ...
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(-89));
}

TEST_F(SailTest, Should_gybe_sail_if_more_than_10_degress_when_running) {
  sail.set_position(0); // to foil anti-gybe
  sail.set_position(168);
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(81));
  sail.set_position(-168);
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(-81));
}

TEST_F(SailTest, Should_set_sail_to_30_degrees_if_windsensor_not_working) {
  sail.set_position(ANGLE_ERROR); // see Windsensor.h
  EXPECT_EQ(stub_servo.angle_last_called,servo_position(30));
}

// TEST_F(SailTest, Print_out_wind_and_sail_values_not_arduino) {
//   for (int i=-180; i<=180; i++) {
//     sail.set_position(i);
//     int s = stub_servo.angle_last_called; // see Windsensor.h
//     std::cout << i;
//     std::cout << ' ';
//     std::cout << s;
//     std::cout << '\n';
